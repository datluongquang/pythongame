...
class Tank(pygame.sprite.Sprite):
    ...
    def shoot(self):
        if self.last_shoot_counter >= FPS/2:
            if self.orientation == "R":
                bullet = Bullet(self.rect.x + BLOCK_SIZE + 2, self.rect.y + BLOCK_SIZE/2 - Bullet.x_size / 2, bullet_img, 10, 0)
            elif self.orientation == "L":
                bullet = Bullet(self.rect.x - 2, self.rect.y + BLOCK_SIZE/2 - Bullet.x_size / 2, bullet_img, -10, 0)
            elif self.orientation == "D":
                bullet = Bullet(self.rect.x + BLOCK_SIZE/2 - Bullet.x_size / 2, self.rect.y  + BLOCK_SIZE + 2, bullet_img, 0, 10)
            else: # up
...
class TankAI(Tank):
    ...
    def __init__(self, x, y, orientation, state):
        super(TankAI, self).__init__(x, y, orientation)
    ...
    def findShortestPathRecursive(self, map, tank_pos, queue, already_traversed):
        ...
        if source_column + 1 < width and map[source_row][source_column+1] == 0 and (source_column+1, source_row) not in already_traversed:
            self.shortest_path_tree[(source_column+1, source_row)] = latest_distance + 1
        ...
        if source_column - 1 < width and map[source_row][source_column-1] == 0 and (source_column-1, source_row) not in already_traversed:
            ...
            queue.put((source_column-1, source_row))
            already_traversed.add((source_column-1, source_row))
        if source_row + 1 < height and map[source_row+1][source_column] == 0 and (source_column, source_row+1) not in already_traversed:
            self.shortest_path_tree[(source_column, source_row+1)] = latest_distance + 1
